...
MyCompany.Aries.Debug = MyCompany.Aries.Debug or {};
local AdvItemViewPage = {};
commonlib.setfield("MyCompany.Aries.Debug.AdvItemViewPage", AdvItemViewPage);

-- The data source for items
function AdvItemViewPage.DS_Func_Items(dsTable, index, pageCtrl)      
    ...
    if(not dsTable.status) then
        ...
        if(index == nil) then
			dsTable.Count = 100;
			AdvItemViewPage.GetItems(class, subclass, bag, pageCtrl, "access plus 5 second", dsTable);
...
function AdvItemViewPage.GetItems(class, subclass, bag, pageCtrl, cachepolicy, output)
	...
	for _, bag in ipairs(bags) do
		ItemManager.GetItemsInBag(bag, "ariesitemsadv_"..bag, function(msg)
